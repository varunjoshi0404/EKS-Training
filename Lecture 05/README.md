...
Certainly. Here's the improved section with added guidance on running commands with a specific profile and how to set `eks-training` as the default profile:
...
**Option 2 (Permanent - update config file):**

You can also update your AWS config file (typically located at `~/.aws/config`) to include:
...
Then copy the credentials from the `[eks-training]` section in `~/.aws/credentials` to the `[default]` section if needed.

This ensures `eks-training` behaves as the default profile across tools like `eksctl`, `kubectl`, and Terraform.
...
Certainly, here’s the updated format for your course notes:
...
> **Important**: This command deletes the control plane and any node groups associated with the cluster. Always verify the cluster name and region before executing the deletion.
